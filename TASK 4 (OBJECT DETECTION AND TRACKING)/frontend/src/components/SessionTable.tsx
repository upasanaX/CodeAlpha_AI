...
export const SessionTable: React.FC<SessionTableProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-[#111622] border border-gray-200 dark:border-gray-800 rounded-lg shadow-xs overflow-hidden transition-colors duration-200">
      ...
      <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between bg-white dark:bg-[#111622]">
      ...
      <div className="overflow-x-auto">
        {sessions.length === 0 ? (
          <div className="py-12 text-center text-gray-400 dark:text-gray-500">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">No sessions recorded yet.</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Start a webcam stream or upload a video to generate session history.</p>
          </div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800/80 bg-white dark:bg-[#111622]">
              {sessions.map((s) => (
                <tr key={s.id} className="hover:bg-brand-50/30 dark:hover:bg-brand-950/20 transition-colors">
                  <td className="py-3 px-4 font-mono font-semibold text-gray-900 dark:text-gray-100">
                    #{s.id}
                  </td>
                  ...
                  <td className="py-3 px-4 text-right">
                    <button
                      ...
                      className="inline-flex items-center space-x-1 text-xs font-semibold text-brand-600 dark:text-brand-400 hover:text-brand-800 dark:hover:text-brand-300 hover:underline transition-colors"
                    >
